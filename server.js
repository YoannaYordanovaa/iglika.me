...
app.get("/getHealth", (req, res) => {
  let sql =
    'SELECT * FROM forever_table WHERE category = "напитки" OR category = "добавки"';
  db.query(sql, (err, results) => {
    if (err) throw err;
    res.send(results);
  });
...
app.get("/getCosmetics", (req, res) => {
  let sql =
    'SELECT * FROM forever_table WHERE category = "Грижа за лицето" OR category = "Грижа за тялото" OR category = "Лична хигиена"';
...
app.post("/submit-form", upload.none(), (req, res) => {
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      ...
      res
        .status(200)
        .json({ message: "Съобщението Ви беше изпратено успешно!" });
    }
  });
});
